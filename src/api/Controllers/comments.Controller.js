...
const POST = async (req, res) => { 
	try{
		...
		if(!(await model.checkUser({ comment_user_id: req?.body?.comment_user_id ? req?.body?.comment_user_id  : req.user.user_id }))?.length) throw new Error("User not found")

		const comment = await model.POST({ ...req.body , comment_user_id: req?.body?.comment_user_id ? req?.body?.comment_user_id  : req.user.user_id})
